...
<!--						      <p>-->
				<select id="tshirttype">                        
					...
					$sql = "SELECT pro_img, pro_category FROM product";

					$result = $conn->query($sql);

					if ($result->num_rows > 0) {
						// แสดงตัวเลือกหากมีข้อมูล
						while($row = $result->fetch_assoc()) {
							$tshirt_image = $row["pro_img"];
							$tshirt_type = $row["pro_category"];
							// อ่านและแปลงข้อมูลรูปภาพเป็น Base64
							
							echo '<option value="data:image/jpeg;base64,' . base64_encode($tshirt_image) . '" >' . $tshirt_type . '</option>';
						}
					} else {
				    	...
				    	<div class="well">
							...
							<div id="avatarlist">
				                ...
				                <?php
									...
									// สร้างคำสั่ง SQL เพื่อดึงข้อมูลรูปภาพ
									$sql = "SELECT spDesign_customScreen FROM screenprintdesign"; // เปลี่ยน id เป็นรหัสของรูปภาพที่ต้องการดึง

									$result = $conn->query($sql);

									if ($result->num_rows > 0) {
										// แสดงรูปภาพหากมีข้อมูล
										while($row = $result->fetch_assoc()) {
										  $image_data = $row["spDesign_customScreen"];
										  echo '<div id="avatarlist">';
										  echo '<img style="cursor:pointer;" class="img-polaroid" src="data:image/jpeg;base64,' . base64_encode($image_data) . '">';
										  echo '</div>';
										}
									  } else {
										echo "ไม่พบรูปภาพ";
		    ...
		    <div class="span6">		    
                ...
                <button id="flipback" type="button" class="btn" title="Rotate View"><i class="icon-retweet" style="height:19px;"></i></button>
					<div id="shirtDiv" class="page" style="width: 530px; height: 630px; position: relative; background-color: rgb(255, 255, 255);">
						...
						<button onclick="saveImageData()">บันทึกข้อมูล</button>
					</div>
   ...
   $("#tshirttype").change(function(){
	...
	<script>
		function saveImageData() {
			var shirtImageSrc = document.getElementById('tshirtFacing').src;
			var canvasData = document.getElementById('tcanvas').toDataURL();

			var xhr = new XMLHttpRequest();
			var url = "save_data.php"; // URL ของไฟล์ PHP ที่ใช้ในการบันทึกข้อมูล
			...
			var data = "shirt_image=" + encodeURIComponent(shirtImageSrc) + "&canvas_data=" + encodeURIComponent(canvasData);
